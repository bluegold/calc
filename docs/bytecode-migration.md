...
- 既定を VM に変更
- tree-walk を fallback として維持する
- 次の大きな言語要素追加で tree-walk 側の保守コストや破綻が見えるまでは削除しない
- 破綻点が明確になった段階で、旧実装の段階削除を判断する
...
3. Feature Flag
- 環境変数方式を採用する。
- `CALC_EXECUTER_MODE=tree|vm` を想定する。
- Phase 0 時点では既定値を `tree` とし、Phase 4 で `vm` へ切り替える。
...
- VM 既定化後も tree-walk fallback を維持する
- 少なくとも次の大きな言語要素追加で tree-walk の追従が難しくなるまでは残す
...
- Phase 4 は完了。既定値を `vm` に切り替え、`CALC_EXECUTER_MODE=tree` を fallback として残している
- tree-walk は次の大きな言語要素で明確な破綻や過大な保守コストが出るまで維持する方針とする
