...
VM 命令の実行を追いたいときは、stderr にトレースを出せます。

```bash
bin/calc --trace-vm --print-last-result samples/recursion.calc
```
...
トレースは bytecode の命令番号（`bc[0001]` など）を含むため、`:bytecode` の表示と対応づけながら追跡できます。TTY では call/jump/store などを色付きで強調表示します。

### Ruby JIT

Ruby の YJIT を有効にして実行できます。

```bash
RUBYOPT=--yjit bin/calc --print-last-result samples/recursion.calc
```

短い単発実行では差が出にくいことがありますが、`fib` のような再帰中心の重めの処理では有利になることがあります。
